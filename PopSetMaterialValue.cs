...
public class PopSetMaterialValue : MonoBehaviour
{
	...
	public void SetTextures(Texture[] Values, string[] Uniforms)
	{
		if (GlobalUniform)
		{
			throw new System.Exception("Not currently supporting SetTextures + GlobalUniform");
		...
		else
		{
			ForEachMaterial((m) => {
				...
				for (var i = 0; i < Mathf.Min(Values.Length, Uniforms.Length); i++)
				{
					var Value = Values[i];
